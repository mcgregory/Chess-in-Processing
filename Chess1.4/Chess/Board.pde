...
public class Board {
       ...
       boolean clickedYet, selected, showCheck, attackSetYet = false;
       int selx, sely = -1;
...
//---------------------------------------------------------------------------------------------------
       ...
       void draw(){    
              ...
              for ( int i=0; i<8; i++ ) {
               ...
               //check if color's king in check
...
//---------------------------------------------------------------------------------------------------              
       public void drawPromotionPieces(){
                ...
                fill(50);
                stroke(225);
                rect( (8*squareSize)+3, (5/2*squareSize), squareSize -6, 4*squareSize);
                
                //draw the pieces for the user to choose from:
                pieces[32].drawAnyPiece(pieces[firstClickPN].getPWhite(), 2,check, 8, 2);
                pieces[32].drawAnyPiece(pieces[firstClickPN].getPWhite(), 3,check, 8, 3);
                pieces[32].drawAnyPiece(pieces[firstClickPN].getPWhite(), 4,check, 8, 4);
                pieces[32].drawAnyPiece(pieces[firstClickPN].getPWhite(), 5,check, 8, 5);
...
//methods       
      ...
      public void display(String s){
          ...
          fill(255);
          if(cnt != 0){
            text(s, 8*check+5, 3 + (cnt*check/2), check-check/15, ((cnt)*check/2) -3 );
          }
          else{
            text(s, 8*check+5, 3 + (cnt*check/2), check-check/15, check/3 -3 );
          }
          cnt++;
      ...
      public void displayWhosTurn(String s){
          ...
          fill(75);
          stroke(200);
          rect( (8*check)+2, check+5, check, check/2);  
        
          fill(255);
          text(s, 8*check+10, check+20, check, check);
                         ...
                         if(k<16 || k>23){
                               ...
                               while (!ischeck && i<21){
                                     if (pieces[20].getColumn() ==  pieces[k].attackedSquares[i][0] && pieces[20].getRow() == pieces[k].attackedSquares[i][1]){
                                          ischeck = true;println("white king in check");
                                          return true;
                                     }
                                     else {
                                          i++;
               ...
               for ( int i=0; i<8; i++ ) {
                   if(pieces[28].attackedSquares[i][0] != -1){
                      if(moveIsValid(pieces[28].attackedSquares[i][0], pieces[28].attackedSquares[i][1], 28, false)){   
                        println("king move is valid for move: " + pieces[28].attackedSquares[i][0] +","+ pieces[28].attackedSquares[i][1]);
...
//-------------------------------------------------------------------------------------------------------- 
   ...
   //basically the tryMove method but does not actually do the move
       public boolean allowMove(int x,int y,int firstClickPN){
                   ...
                   if(!isEmptyAt(x,y )){ 
                        pieceNum = getPieceNum(x , y ); 
                        pieces[pieceNum].setRow(-1);
                        pieces[pieceNum].setColumn(-1);    
                   }
...
//-------------------------------------------------------------------------------------------------------- 
   //this method is used after boolean moveIsValid and does several checks for the move
       public void tryMove(int x,int y,int firstClickPN){
                   ...
                   if(!isEmptyAt(x,y )){ 
                        ...
                        pieces[pieceNumAttacked].setRow(-1);
                        pieces[pieceNumAttacked].setColumn(-1);    
                   }
                   pieces[firstClickPN].setRow(y);
                   pieces[firstClickPN].setColumn(x);
...
//-------------------------------------------------------------------------------------------------------- 
       ...
       public void mouseClicked() {
                ...
                //need to check if a pawn has triggered flickOn in Piece, if so this click is for promotion (else)
                if(promotion == false){
